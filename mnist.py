...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        ##############################################################################
        # forward() 는 [input layer] -> [hidden layer] -> ... -> [hidden layer] -> [output layer]
        # 로 각 퍼셉트론을 연결하여 Multi Layered Perceptron Model을 구현한다.
        #
        # forward() 함수는 파이썬 내부적으로 __call__ 매직변수를 사용하여 구현됐다.
        # 따라서, 별도의 호출없이 instance를 생성하기만 하면 알아서 호출되어 실행된다.
        #
        # 정리하면, forward() 함수의 역할은 입력 데이터들이 어떤 형태의 히든 레이어들을 거쳐서
        # 출력하게 할 것인지 그 구조를 정의하는 것이고, 딥러닝의 목적인 적절한 가중치 행렬 w를 찾는 것은
        # backward() 함수가 역전파 알고리즘을 이용해 처리해준다.
        ##############################################################################
...
def main():
    ...
    print(f"Total training and evaluation time: {total_time:.4f} 초")
